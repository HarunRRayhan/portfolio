...
# Generate a timestamp directly in this script
TIMESTAMP=$(date +%Y%m%d%H%M%S)
# Pass the timestamp to the docker-compose file via environment variable
export TIMESTAMP

# Run the prepare-docker.sh script to set up Docker configuration
execute_ssh "cd $APP_DIR && TIMESTAMP=$TIMESTAMP ${SCRIPT_DIR}/prepare-docker.sh"
if [ $? -ne 0 ]; then
