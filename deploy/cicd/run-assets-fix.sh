...
# Script to safely run the assets fix script on the remote server
...
echo "Copying assets fix script to server..."
scp -i "$DEPLOY_DIR/$SSH_KEY" "$SCRIPT_DIR/fix-assets-server.sh" "$REMOTE_USER@$REMOTE_HOST:/tmp/fix-assets-server.sh"

# Make the script executable and run it with sudo
echo "Running assets fix script on server..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "chmod +x /tmp/fix-assets-server.sh && sudo /tmp/fix-assets-server.sh"
...
# Clean up
echo "Cleaning up..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "rm -f /tmp/fix-assets-server.sh"
