...
# Script to safely run the build assets fix script on the remote server
...
echo "Copying build assets fix script to server..."
scp -i "$DEPLOY_DIR/$SSH_KEY" "$SCRIPT_DIR/fix-build-assets.sh" "$REMOTE_USER@$REMOTE_HOST:/tmp/fix-build-assets.sh"

# Make the script executable and run it with sudo
echo "Running build assets fix script on server..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "chmod +x /tmp/fix-build-assets.sh && sudo /tmp/fix-build-assets.sh"
...
# Clean up
echo "Cleaning up..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "rm -f /tmp/fix-build-assets.sh"
