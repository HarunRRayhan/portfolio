...
# Script to safely run the container environment fix script on the remote server
...
echo "Copying container fix script to server..."
scp -i "$DEPLOY_DIR/$SSH_KEY" "$SCRIPT_DIR/fix-container-env.sh" "$REMOTE_USER@$REMOTE_HOST:/tmp/fix-container-env.sh"

# Make the script executable and run it with sudo
echo "Running container fix script on server..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "chmod +x /tmp/fix-container-env.sh && sudo /tmp/fix-container-env.sh"
...
# Clean up
echo "Cleaning up..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "rm -f /tmp/fix-container-env.sh"
