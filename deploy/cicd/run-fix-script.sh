...
# Script to safely run the Cloudflare 521 fix script on the remote server
# This avoids exposing sensitive credentials in the command history
...
scp -i "$DEPLOY_DIR/$SSH_KEY" "$SCRIPT_DIR/fix-cloudflare-521.sh" "$REMOTE_USER@$REMOTE_HOST:/tmp/fix-cloudflare-521.sh"

# Make the script executable and run it with sudo
echo "Running fix script on server..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "chmod +x /tmp/fix-cloudflare-521.sh && sudo /tmp/fix-cloudflare-521.sh"
...
# Clean up
echo "Cleaning up..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "rm -f /tmp/fix-cloudflare-521.sh"
