...
# Script to safely run the Laravel configuration fix script on the remote server
...
echo "Copying Laravel configuration fix script to server..."
scp -i "$DEPLOY_DIR/$SSH_KEY" "$SCRIPT_DIR/fix-laravel-config.sh" "$REMOTE_USER@$REMOTE_HOST:/tmp/fix-laravel-config.sh"

# Make the script executable and run it with sudo
echo "Running Laravel configuration fix script on server..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "chmod +x /tmp/fix-laravel-config.sh && sudo /tmp/fix-laravel-config.sh"
...
TEST_RESULT=$(ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "curl -s http://localhost/test.php || echo 'Connection failed'")
echo "PHP test result: $TEST_RESULT"
...
# Clean up
echo "Cleaning up..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "rm -f /tmp/fix-laravel-config.sh"
