...
# Script to safely run the Laravel permissions fix script on the remote server
...
echo "Copying permissions fix script to server..."
scp -i "$DEPLOY_DIR/$SSH_KEY" "$SCRIPT_DIR/fix-permissions.sh" "$REMOTE_USER@$REMOTE_HOST:/tmp/fix-permissions.sh"

# Make the script executable and run it with sudo
echo "Running permissions fix script on server..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "chmod +x /tmp/fix-permissions.sh && sudo /tmp/fix-permissions.sh"
...
# Clean up
echo "Cleaning up..."
ssh -i "$DEPLOY_DIR/$SSH_KEY" "$REMOTE_USER@$REMOTE_HOST" "rm -f /tmp/fix-permissions.sh"
