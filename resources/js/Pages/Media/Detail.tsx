...
import { ArrowLeft, ArrowRight, CalendarDays, ExternalLink, MonitorPlay, Plane, Presentation, Share2 } from 'lucide-react'
...
const copyByType = {
  slide: {
    ...
    pairedKicker: 'Same talk, another format',
    pairedHeading: 'Want to watch the talk?',
    pairedCta: 'Watch the video',
  },
  video: {
    ...
    pairedKicker: 'Same talk, another format',
    pairedHeading: 'Want the slide deck?',
    pairedCta: 'View the slides',
  },
} as const

export default function MediaDetailPage({ type, item, paired, related, canonicalUrl }: Props) {
      ...
      <div className="pt-24">
        <section id={item.slug} className="mx-auto max-w-5xl px-4 pb-16 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 max-w-3xl">
            ...
            <div className="mt-6">
              <ShareButton
                url={item.pageShareUrl}
                title={item.title}
                shareTitle={item.title}
                label={`Share "${item.title}"`}
                theme="slate"
                triggerClassName="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-100 hover:text-slate-950"
              >
          ...
          {paired ? (
            <section className="mt-10 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{copy.pairedKicker}</p>
              <div className="mt-3 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div className="max-w-2xl">
                  <h2 className="text-2xl font-semibold tracking-tight text-slate-950">{copy.pairedHeading}</h2>
                  <p className="mt-2 text-sm leading-7 text-slate-600">{paired.title}</p>
                  {paired.summary ? <p className="mt-2 text-sm leading-7 text-slate-500">{paired.summary}</p> : null}
                </div>
                <Link
                  href={paired.detailUrl}
                  className="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-slate-900 transition-colors hover:text-slate-600"
                >
                  {copy.pairedCta}
