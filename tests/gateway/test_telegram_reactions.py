...
async def test_set_reaction_calls_bot_api(monkeypatch):
    ...
    result = await adapter._set_reaction("123", "456", "\U0001f44d")
...
async def test_on_processing_start_adds_thumbs_up_reaction(monkeypatch):
    """Processing start should add a thumbs-up reaction when enabled."""
    monkeypatch.setenv("TELEGRAM_REACTIONS", "true")
    adapter = _make_adapter()
    event = _make_event()
...
@pytest.mark.asyncio
async def test_on_processing_complete_success_noop(monkeypatch):
    """Successful processing should leave the thumbs-up reaction in place."""
    monkeypatch.setenv("TELEGRAM_REACTIONS", "true")
    adapter = _make_adapter()
    event = _make_event()
...
@pytest.mark.asyncio
async def test_on_processing_complete_failure_noop(monkeypatch):
    """Failed processing should not overwrite the receipt acknowledgment."""
    monkeypatch.setenv("TELEGRAM_REACTIONS", "true")
    adapter = _make_adapter()
    event = _make_event()
...
@pytest.mark.asyncio
async def test_on_processing_complete_cancelled_noop(monkeypatch):
    """Cancelled processing should not remove the receipt acknowledgment."""
    monkeypatch.setenv("TELEGRAM_REACTIONS", "true")
    adapter = _make_adapter()
    event = _make_event()
...
def test_config_bridges_telegram_reactions(monkeypatch, tmp_path):
    ...
    # the var doesn't exist yet, and load_gateway_config will overwrite it.
